...
export async function readSession(): Promise<PersistedSession | null> {
  if (canUseSecureStore) {
    const secureRaw = await SecureStore.getItemAsync(SESSION_STORAGE_KEY).catch(() => null);
...
export async function writeSession(session: PersistedSession): Promise<void> {
  ...
  if (canUseSecureStore) {
    await SecureStore.setItemAsync(SESSION_STORAGE_KEY, payload).catch(() => null);
  }
  await AsyncStorage.setItem(SESSION_STORAGE_KEY, payload).catch(() => null);
}

export async function clearSession(): Promise<void> {
  if (canUseSecureStore) {
    await SecureStore.deleteItemAsync(SESSION_STORAGE_KEY).catch(() => null);
  }
