...
type Tab = 'chats' | 'friends' | 'family' | 'profile';
      ...
      ListEmptyComponent={
        <View style={[styles.empty, styles.emptyCove]}>
          ...
          {emptyActionEnabled ? (
            ...
            >
              <Text style={styles.btnText}>{emptyActionLabel || firstMessageLabel}</Text>
            </Pressable>
          ) : null}
        </View>
      }
...
function App() {
  ...
  const [roomMessageError, setRoomMessageError] = useState('');
  ...
  const applyPersistedAppSnapshot = (snapshot: PersistedAppSnapshot | null) => {
    ...
    if (snapshot.tab === 'friends' || snapshot.tab === 'chats' || snapshot.tab === 'family' || snapshot.tab === 'profile') {
      setTab(snapshot.tab);
    }
  ...
  const favoriteDirectRooms = useMemo(() => filteredRooms.filter((room) => room.favorite), [filteredRooms]);
  const otherDirectRooms = useMemo(() => filteredRooms.filter((room) => !room.favorite), [filteredRooms]);
  ...
  const familyTabRooms = useMemo(() => sortedRooms.filter((room) => room.type === 'family'), [sortedRooms]);
  ...
  const activeFriendsTabLabel = s.tabsFriends;
  const chatsTabLabel = s.tabsChats;
  ...
  const mapBackendMessage = (message: BackendRoomMessage): Message => {
    ...
    const mine = !!senderId && senderId === currentUserIdRef.current;
    const messageKind = message.kind || message.type || (message.uri || message.mediaUrl || message.url ? 'image' : 'text');
    const messageText = message.text ?? message.body ?? message.content;
    const messageUri = message.uri ?? message.mediaUrl ?? message.url;
    const messageAt = message.at ?? message.createdAt ?? message.timestamp ?? message.updatedAt;
    return {
      id: String(message.id || message.messageId || message.clientMessageId || uid()),
      roomId: String(message.roomId || activeRoomId || ''),
      senderId,
      senderName: String(
        message.senderName || message.sender?.name || message.sender?.displayName || getFriend(senderId)?.name || getBot(senderId)?.name || profile.name || s.me
      ),
      mine,
      kind: messageKind as MsgKind,
      ...(messageText ? { text: String(messageText) } : {}),
      ...(messageUri ? { uri: resolveBackendMediaUrl(String(messageUri)) } : {}),
      at: parseTimestamp(messageAt),
...
*/
  ...
  const syncRoomMessagesFromBackend = async (token: string, roomId: string): Promise<Message[]> => {
    ...
    const items = asListItems<BackendRoomMessage>(raw).filter(
      (item) => !!(item?.id || item?.messageId || item?.clientMessageId)
    );
  ...
  const openRoom = (rid: string) => {
    ...
    if (token) {
      ...
      void syncRoomReadState(token, rid, { refreshMessages: true })
        .catch((err) => {
          const msg = err instanceof Error ? err.message : '';
          setRoomMessageError(normalizeBackendErrorMessage(msg || s.loginBackendSyncFailed, isKo));
        })
        .finally(() => {
                ...
                {false ? <ScrollView contentContainerStyle={styles.list}>
                  ...
                  <ChatMessageList
                    ...
                    noMessageBody={
                      isRoomMessagesLoading
                        ? isKo
                          ? '서버에서 대화 내용을 동기화하고 있어요.'
                          : 'Syncing this room from the server.'
                        : roomMessageError || s.denQuietNote
                    }
                    firstMessageLabel={s.firstMsg}
                    emptyActionEnabled={!isRoomMessagesLoading}
                    emptyActionLabel={
                      roomMessageError
                        ? isKo
                          ? '다시 시도'
                    ...
                    onEmptyAction={
                      roomMessageError
                        ? () => {
                            if (activeRoom) openRoom(activeRoom.id);
                          }
                        : undefined
                    }
                    helloSeed={s.hello}
            ...
            <>
              <View style={styles.header}>
                ...
                {tab === 'friends' ? (
                  ...
                  <Pressable style={styles.iconDark} onPress={() => openCreateRoomModal('family')}>
                    <Ionicons name="shield-checkmark" size={18} color={FOREST.text} />
                  </Pressable>
                ) : (
                  <Pressable
              ...
              <View style={styles.main}>
                      ...
                      <>
                        {true ? (
                          <>
                            ...
                            {otherDirectRooms.length > 0 ? <Text style={styles.sectionTitle}>{isKo ? '대화방' : 'Rooms'}</Text> : null}
                ...
                {tab === 'family' ? (
                  <ScrollView contentContainerStyle={styles.list}>
                    <View style={[styles.profileCabinCard, styles.emptyCove]}>
                      <View style={styles.profileCabinText}>
                        <Text style={styles.h1}>{isKo ? '가족 그룹' : 'Family groups'}</Text>
                        <Text style={styles.sub}>
                          {isKo
                            ? '가족 구성과 자녀 위치 확인은 가족방 기준으로 연결돼요.'
                            : 'Family structure and child location checks are connected through family rooms.'}
                        </Text>
                      </View>
                      <View style={styles.row}>
                        <Pressable style={styles.smallBtn} onPress={() => openCreateRoomModal('family')}>
                          <Text style={styles.smallBtnText}>{s.createFamilyRoomAction}</Text>
                        </Pressable>
                        <Pressable style={styles.smallBtn} onPress={() => void syncLocationPermissionState(true)}>
                    ...
                    <View style={styles.appUpdateCard}>
                      <Text style={styles.sectionTitle}>{isKo ? '위치 추적 상태' : 'Location status'}</Text>
                      <Text style={styles.sub}>
                        {profile.locationSharingEnabled
                          ? isKo
                            ? '내 최근 위치가 가족방에서 확인 가능하도록 동기화돼요.'
                            : 'Your latest location can be synced for family rooms.'
                          : isKo
                            ? '위치 공유를 켜야 가족방에서 자녀 위치 확인과 요청을 사용할 수 있어요.'
                            : 'Turn on location sharing to use family location checks and requests.'}
                      </Text>
                      <Text style={styles.sub}>
                        {locationPermissionGranted
                          ? isKo
                            ? '기기 위치 권한이 허용되어 있어요.'
                            : 'Device location permission is allowed.'
                          : isKo
                            ? '기기 위치 권한이 아직 꺼져 있어요.'
                            : 'Device location permission is off.'}
                      </Text>
                      <View style={styles.row}>
                        <Pressable
                          style={[styles.smallBtn, isUpdatingLocationSharing && styles.off]}
                          disabled={isUpdatingLocationSharing}
                          onPress={() => void syncLocationPermissionState(true)}
                        >
                          <Text style={styles.smallBtnText}>
                        ...
                        {profile.locationSharingEnabled ? (
                          <Pressable
                            style={[styles.smallBtn, isUpdatingLocationSharing && styles.off]}
                            disabled={isUpdatingLocationSharing}
                            onPress={() => void disableLocationSharing()}
                          >
                            <Text style={styles.smallBtnText}>{isKo ? '위치 공유 끄기' : 'Turn off sharing'}</Text>
                          </Pressable>
                        ) : null}
                      </View>
                    </View>

                    {familyTabRooms.length === 0 ? (
                      <View style={[styles.empty, styles.emptyCove]}>
                        <Text style={styles.h1}>{isKo ? '가족방이 없어요' : 'No family rooms yet'}</Text>
                        <Text style={styles.sub}>
                          {isKo
                            ? '가족방을 만들면 구성원 별칭, 보호자/자녀 관계, 최근 위치를 여기서 관리할 수 있어요.'
                            : 'Create a family room to manage aliases, guardian-child links, and recent locations here.'}
                        </Text>
                        <Pressable style={styles.smallBtn} onPress={() => openCreateRoomModal('family')}>
                          <Text style={styles.smallBtnText}>{s.createFamilyRoomAction}</Text>
                        </Pressable>
                      ...
                      <>
                        <Text style={styles.sectionTitle}>{isKo ? '가족방' : 'Family rooms'}</Text>
                        {familyTabRooms.map((room) => (
                          <View key={`family-tab-${room.id}`} style={styles.familyStructureCard}>
                            <View style={styles.familyStructureHeader}>
                              <View style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <Text style={styles.sub} numberOfLines={2}>
                                  {roomMemberNames(room) || (isKo ? '가족 구성원을 확인해 주세요.' : 'Review family members.')}
                                </Text>
                              </View>
                              {room.unread > 0 ? (
                                <View style={styles.badge}>
                            ...
                            <View style={styles.row}>
                              <Pressable style={styles.smallBtn} onPress={() => void openFamilyStructureEditor(room.id)}>
                                <Text style={styles.smallBtnText}>{isKo ? '구성/위치 보기' : 'Structure & location'}</Text>
                              </Pressable>
                              <Pressable style={styles.smallBtn} onPress={() => openRoom(room.id)}>
                                <Text style={styles.smallBtnText}>{s.tabsChats}</Text>
                              </Pressable>
                            </View>
                          </View>
                        ))}
              ...
              <View style={styles.tabs}>
                ...
                <Pressable style={[styles.tab, tab === 'family' && styles.tabOn]} onPress={handleFamilyTabPress}>
                  <Ionicons name="shield-checkmark" size={18} color={tab === 'family' ? FOREST.text : FOREST.textMuted} />
                  <Text style={styles.tabText}>{s.tabsFamily}</Text>
                </Pressable>
