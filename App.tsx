...
const normalizeBackendErrorMessage = (message: string, isKo: boolean): string => {
  ...
  const map: Array<[string, string, string]> = [
    ['The other participant already left this room.', '상대방이 이미 이 대화방을 나갔어요.', 'The other participant already left this room.'],
    ['Use DELETE /rooms/:roomId for direct rooms.', '1:1 대화방은 나가기만 할 수 있어요.', 'Direct rooms can only be left.'],
    ['Room has no active members.', '현재 참여 중인 사용자가 없는 대화방이에요.', 'This room has no active members.'],
    ['Room not found.', '대화방을 찾을 수 없어요.', 'Room not found.'],
    ['Target user not found.', '대상 사용자를 찾을 수 없어요.', 'Target user not found.'],
    ['Direct room can be opened only with friends (or active bot accounts).', '친구와만 1:1 대화를 열 수 있어요.', 'Direct rooms can only be opened with friends.'],
    ['Group title must be 1-100 characters.', '그룹 이름은 1자 이상 100자 이하로 입력해 주세요.', 'Group title must be between 1 and 100 characters.'],
    ['Group room requires at least 2 members.', '그룹방은 최소 2명 이상이어야 해요.', 'Group rooms require at least 2 members.'],
  ];
